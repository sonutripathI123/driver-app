...
export const Header: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-30 h-18 w-full bg-[#0A0E1A]/90 backdrop-blur-md border-b border-[#1F2E4D] px-8 flex items-center justify-between">
      {/* Left: Melbourne Hub Telemetry & Link */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-[#121A2D] border border-[#1F2E4D] text-xs">
          <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-sm shadow-emerald-400 animate-pulse" />
          <span className="font-semibold text-slate-200">Melbourne Hub</span>
          <span className="text-slate-500">|</span>
          <span className="font-mono text-slate-300 font-bold">{timeStr} AEST</span>
        </div>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="hidden md:flex items-center gap-1 text-xs text-slate-400 hover:text-amber-400 transition-colors"
        >
      ...
      <div className="flex items-center gap-3">
        {/* Engine Status Badge */}
        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#121A2D] border border-amber-500/40 text-amber-300 text-xs font-bold shadow-sm shadow-amber-500/10">
          <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-spin" style={{ animationDuration: '8s' }} />
          <span>FastAPI 2.0 Engine</span>
        ...
        <div className="relative group">
          <button className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-semibold transition-all ${roleLabels[currentRole].color}`}>
            <Shield className="w-3.5 h-3.5" />
            <span>{roleLabels[currentRole].label}</span>
            <ChevronDown className="w-3 h-3 ml-1 opacity-70 group-hover:rotate-180 transition-transform" />
          </button>

          {/* Role Dropdown */}
          <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-[#0D1322] border border-[#1F2E4D] shadow-2xl p-2 hidden group-hover:block transition-all z-50">
        ...
        <button className="relative p-2.5 rounded-xl bg-[#121A2D] border border-[#1F2E4D] text-slate-300 hover:text-amber-400 hover:border-amber-500/40 transition-colors">

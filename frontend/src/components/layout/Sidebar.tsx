...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-[#06090F] border-r border-[#1E2738] select-none text-white">
      {/* Brand Header */}
      <div className="p-6 flex items-center justify-between border-b border-[#1E2738] bg-[#06090F]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white font-mono tracking-widest uppercase font-bold opacity-90">
        ...
        {isOpenMobile && (
          <button
            ...
            className="lg:hidden p-1.5 rounded-lg bg-[#121824] text-white hover:bg-[#1A2233] transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
        {sections.map((section) => (
          <div key={section.title} className="space-y-1.5">
            <h3 className="px-3 text-[10px] font-black tracking-wider text-white uppercase opacity-80">
              {section.title}
            </h3>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all group ${
                      isActive
                        ...
                        : 'text-white hover:text-white hover:bg-[#121824]'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <Icon
                        className={`w-4 h-4 transition-transform group-hover:scale-110 ${
                          isActive ? 'text-[#0A0E1A]' : 'text-white'
                        }`}
                      />
                      <span className={isActive ? 'text-[#0A0E1A] font-black' : 'text-white font-bold'}>{item.label}</span>
                    </div>
                      ...
                      <span
                        className={`px-2 py-0.5 text-[9px] font-black rounded-full border ${
                          isActive
                            ...
                            : 'bg-[#FAF6F0] text-[#0A0E1A] border-[#E6D8C3]'
                        }`}
                      >
                        {item.badge}
                      </span>
